...
{	
		...
		$adressArray=array("Country"=>$Country,"landmark"=>$landmark,"State"=>$State,"City"=>$City,"Address"=>$Address,"Locality"=>$Locality,"ZIP"=>$ZIP,"Phone"=>$Phone,"Name"=>$Name,"userId"=>$userId,"USER_ADDRESS_ID"=>$USER_ADDRESS_ID,"eu_vat"=>$EU_VAT,"company_name"=>$companyName);
		$objUserManager = new UserManager(); 
		
	$result=$objUserManager->InsertUserAddress($adressArray);
	header("location:cart-checkout1.php?urlstring=".EncryptURL("action=&msg=added"));
		break;
		...
		case "checkOut":
			...
			if($currentAddress!="")
			{
				...
				$objProductManager = new ProductManager(); 
				$ProductsDetails=$objProductManager->UpdateProductStatus($userId,$status,$currentAddress);
				$objProductManager->UpdateProductShipingAmt($userId,$_POST["shiping_amt"],$vat_amt);
				...
				if(trim($_POST["paymentMethod"])=='Paypal')
				{
					...
					{
						 header("location:cart-checkout1.php?urlstring=".EncryptURL("action=&msg=error"));
					}
				}
					...
					if($_SESSION['CUSTOMER_EMAIL']!="")
					{
						...
						foreach($orderDetails as $order)
						{
							...
							$totalAmount=$totalAmount+round(($order->PRODUCT_AMT*$order->QUANTITY),2);
								...
								$message.='<tr>
												<td width="10%" colspan="4" align="center" style="background-color:#33CCFF"><strong>VAT(19%)</strong></td>
												<td width="10%" align="center" style="background-color:#33CCFF"><strong>'.round($orderDetails[0]->TAX_TOTAL_AMOUNT,2).'</strong>						                                                </td>
											</tr>';
								$message.='<tr>
								...
								$message.='<tr>
												...
												<td width="10%" align="center" style="background-color:#33CCFF"><strong>'.(round($totalAmount,2)+round($orderDetails[0]->TAX_TOTAL_AMOUNT,2)+round($orderDetails[0]->SHIPING_AMT,2)).'</strong></td>
											</tr>';
								$message.'</table>
									<br/><br/>';
								$message.'<strong>Note:</strong> Kindly do not reply to this email as this is an auto generated email from Sinelec. For any query kindly contact sales@sinelec-tech.com';
				...
				<div class="stepwizard">
					<div class="stepwizard-row">
						...
						<div class="stepwizard-step" style="width:0%">
							<?php 
							if($_SESSION['currentAddress']!="") 
							{
							?>
							<a href="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=PaymentMethod'); ?>">
							<button type="button" class="btn-danger button-circle">3</button></a><?php } else { ?><button type="button" class="btn-danger button-circle">3
							</button>
							<?php 
							}
							?>
		 ...
		 <div class="col-sm-8">
				<div class="row">
					...
					<h2>Delivery Address</h2>
							...
							<form class="login-form clearfix" action="login.php?urlstring=<?php echo EncryptURL('action=Login'); ?>" 
								...
								<div class="row">
									<div class="col-sm-1" >
										...
										onClick="deliveryAddress('<?php echo $val->USER_ADDRESS_ID;?>','<?php echo $val->SHIPPING_AMT ;?>','<?php echo $val->EU_VAT ;?>',
										'<?php echo $val->COUNTRY_ID ;?>','<?php echo $val->COMPANY_NAME ;?>')">
										</div>
										<div class="col-sm-8" >
									...
									<div class="col-sm-1" >
										<a href="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=AddAddress&addressId='.$val->USER_ADDRESS_ID); ?>" 
						  ...
						  <a href="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=AddAddress'); ?>" class="btn btn-border btn-xlg"   >Add new address</a>
					</div>
				</div>
			</div>
			...
			var country = ["11","18","28","45","48","50","59","62","63","71","80","86","88","99","105","106","113","120","128","143","144","148","162","163","169","174","190"];
				...
				function deliveryAddress(id,shiping_amt,eu_vat,countryId,company_name)
				{
					...
					document.getElementById("currentAddress").value=id;
					...
					if(countryId=='68')
						vatAmount = ((Number(shiping_amt)+Number(OrderProductTotalPrice))*19)/100;
					...
					//var withoutVatAmount= parseFloat(Number(shiping_amt)+Number(OrderProductTotalPrice)-deductedAmount).toFixed(2);
...
<!--									<tr>
										...
										<td class="text-right text-muted">
											<small><span class="glyphicon glyphicon-euro"></span> <span id="WITHOUT_VAT_AMT"><?php //if(isset($OrderProductTotalPrice)) echo number_format($OrderProductTotalPrice-($OrderProductTotalPrice*19)/100,2); ?></span></small>
										</td>
									</tr>
-->									<tr>
						...
						{ 
						?>
						 <form action="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=PaymentMethod'); ?>" name="checkOut" id="checkOut" method="post">
							   ...
							   var AddressValue='';
								for(var i = 0; i < CheckBoxAddress.length; i++)
								{
									...
									if(is_true_flag==false)
									{
									 ...
									 document.document.getElementsByName("currentAddress").value=AddressValue;
									return is_true_flag;
						}
					}
					</script>
				...
				<div class="col-sm-12">
					...
					<div class="stepwizard">
						<div class="stepwizard-row">
							...
							<div class="stepwizard-step" style="width:95%">
								 <a href="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=&OrderTotalAmt='.$_POST['total_order_amt']); ?>">
					...
					<table class="table m-bottom0">
						<tbody>
							...
							if($_POST['currentAddress']=="")
								$_POST['currentAddress']=$_SESSION['currentAddress'];
							...
							?>
								<tr>
									<td class="text-right text-muted">
										<input type="radio" name="payMethods" id="payMethod_<?php echo $key ;?>" 
										<?php if($_POST['company_name']=="" && $_POST['vat_amt']=="" && $key=='Invoice') echo 'disabled="disabled"'; ?> 
										style="float:left;" onClick="paymentMethodFun('payMethod_<?php echo $val->COUNTRY_ID ;?>')">
									</td>										
									...
									<th scope="row">
										...
										{
										?><img src="images/Bank-Transfer.png" >
										<?php
										}
										else if($key=='Invoice')
										{ 
							...
							$methodStr=ltrim($methodStr,",");
							?>
							<tr>
								<td> <a href="products.php"> <input class="btn btn-border btn-xlg"  type="button" value="Continue Shopping" /></a></td>
								<td>&nbsp;</td>
								<td>
									 <form action="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=checkOut'); ?>" name="checkOut" id="checkOut" method="post">
										<input type="hidden" name="currentAddress" id="currentAddress" value="<?php echo $_POST['currentAddress'];?>" />
										<input type="hidden" name="userId" id="userId" value="<?php echo $userId;?>" />
									   ...
									   value="Amount to pay <?php echo $_POST['total_order_amt'];?>" onClick="return validatePaymentMethod()" />
									 </form>
								 </td>
							</tr>
						</tbody>
			...
			<div class="col-sm-8">
				 ...
				 <input type="radio" checked="checked" >Add New Address
					 <div class="form text-left">
						 <form class="clearfix  bg-gray p-top30" action="cart-checkout1.php?urlstring=<?php echo EncryptURL('action=Insert'); ?>" method="post" enctype="multipart/form-data">
							...
							<div class='form-row'>
							  ...
							  <div class='col-xs-6 form-group card required'>
								...
								if(count($countryList)>0)
								{
									...
									{ 
								?> 
								<option  value="<?php echo $country->COUNTRY_ID; ?>"  <?php if(isset($userAddress) && $userAddress[0]->COUNTRY_ID==$country->COUNTRY_ID)
							  ...
							  <div class="form-row" id="vatNo">
								  <div class="col-xs-6 form-group  required">
									...
									<input placeholder="EU VAT Number" type="text" name="EU_VAT" id="EU_VAT" value="<?php if(isset($userAddress)) 
									echo $userAddress[0]->EU_VAT; ?>" >
								 </div>
								  <div class="col-xs-6 form-group  required">
									<span style="color:#FF0000" id="note">Note :-If your company is situated in EU but outside of Germany  then by providing your company
									 name and a Valid EU VAT Number you can shop VAT free from us.If you provide Invalid EU VAT Number , your order will be cancelled !!!.</span>
								 </div>
							</div>
							<div class="form-row">
							  <div class="col-xs-12 form-group  required">
