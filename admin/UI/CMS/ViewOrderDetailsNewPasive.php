...
switch($action)
{
	case 'UpdateStatus':
		...
		if($order_status=='Delete Order')
		{
			if($objHomeManager->DeleteOrderByOrderId($order_id))
			{
				...
				header("Location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=delete&OrderNo=".$order_id));
			}
			else
			{
				header("Location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=error"));
			}	
		...
		else
		{
			...
			if(count($arrResult)>0)
			{
				...
				foreach($arrResult as $arrResultVal)
				{
					...
					if (!$mailOb->Send()) 
					{
						//header("Location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=update&OrderNo=".$order_id));
					} 
					$in++;
				}
			}
	...
	case 'ResendInvoice':
		...
		//////////////////billing Address////////////////////////////
			...
			if($orderDetails[0]->COMMUNICATION_EMAIL_ID!='')
			{
				...
				header("location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=resend"));
			}	
		}
		else
		{
	...
	case 'PaymentRemainder':
		...
		if(count($orderDetails)>0)
		{
			...
			if($orderDetails[0]->COMMUNICATION_EMAIL_ID!='')
			{
				...
				header("location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=remainder"));
			}	
		}
		else
		{
	...
	case 'Delete':
		...
		if($objHomeManager->DeleteOrderByOrderId($orderId))
		{
			...
			unlink($fileInvoice[0]);
		  	...
		  	header("location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=delete"));
		}
		else
		{
			header("location:ViewOrderDetailsNewPasive.php?urlstring=".EncryptURL("action=&msg=error"));
	...
	case 'Search':
		...
		header("location:ViewOrderDetailsNewPasive.php?urlstring=". EncryptURL("action=pagging&OrderStatus=".$OrderStatus.'&limit='.$limit.'&maxRecord='.$maxRecord));	
	break;
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="box" id="SibsSchool">
				...
				<div class="box-tools">
				<form  action="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=Search'); ?>" method="post">
                ...
                if($action=="pagging")
                {
                    ...
                    if(!isset($AdminAllDetailsPaging))
                    {
                        ...
                        for($page=1;$page<=$pageCount;$page++)
                        {
                            ...
                            <tr>
                                <td>
                                    <a href="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=pagging&limit='.$limit.'&maxRecord='.$maxRecord);?>" 
                        ...
                        <table>
                            <tr>
                                <td> 
                                 <a href="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=ExportExcel&Status='.$OrderStatus); ?>">
                    ...
                    if(!empty($AdminAllDetailsPaging))
                    {
                        ...
                        foreach($AdminAllDetailsPaging as $value)
                        {
                            ...
                            <tr class="common_table_header">
                                ...
                                {
                                ?>
                                <a href="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=PaymentRemainder&orderId='.$value->ORDER_ID.'&orderStatus='.$value->ORDER_CURRENT_STATUS) ;?>">
                                ...
                                {
                                ?>
                                <a href="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=ResendInvoice&orderId='.$value->ORDER_ID.'&orderStatus='.$value->ORDER_CURRENT_STATUS) ;?>">
                                ...
                                {
                                ?>
                                <a href="ViewOrderDetailsNewPasive.php?urlstring=<?php echo EncryptURL('action=Delete&orderId='.$value->ORDER_ID) ;?>" onClick="return confirm('Are You Sure you want to Delete Order? \n Click OK to Continue, Cancel to Stop');">
                                ...
                                <!--<a href="ViewOrderDetailsNewPasive.php?urlstring=" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure you want to Delete this record ?\n Click OK to Continue, Cancel to Stop')" ><span class="glyphicon glyphicon"  ></span> View details</a>-->
                                </td>	
                            </tr>
                        <?php 
                        }
